...
- `stream-headers-early`: when set, a streaming response commits its HTTP 200 and SSE headers as soon as the request is accepted, before the first token, as vLLM does; a failure that surfaces afterwards is sent in-stream as an error frame followed by `[DONE]`. Off by default: the simulator then holds the status and headers until the first token is ready, so a request that fails before producing content still gets its real HTTP status. Off, a long `time-to-first-token` looks to the client like a backend that never answered; on, it looks like a backend that accepted the request and is queueing or prefilling
- `time-to-headers`: with `stream-headers-early`, the delay before the HTTP status and headers are committed, optional, by default zero. Independent of `time-to-first-token`, so a slow status line and a slow first token can each be simulated on their own
- `stream-abort-after-tokens`: when positive, a streaming response's connection is closed abruptly once this many token chunks have been sent, with no finish chunk, usage chunk or `[DONE]`, and no terminating HTTP chunk: what a client sees when the serving process dies mid-generation. Zero (the default) disables it
