...
- **`POST /admin/config`** applies a partial JSON update and returns the new configuration. The request body must be a JSON object whose keys are a subset of the admin-configurable fields:
  ...
  - `stream-headers-early` — boolean; commit a streaming response's HTTP 200 and SSE headers when the request is accepted instead of holding them until the first token. See [configuration](configuration.md#latency).
  - `time-to-headers` — duration; with `stream-headers-early`, the delay before the status and headers are committed, independent of `time-to-first-token`. Sent at the top level or nested under `latencies` like the other latency fields.
  - `stream-abort-after-tokens` — integer; close a streaming response's connection abruptly after this many token chunks, with no finish chunk or `[DONE]` (a backend dying mid-generation). `0` disables. See [configuration](configuration.md#latency).
